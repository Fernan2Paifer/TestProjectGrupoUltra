...
var ya=false;
    ...
    async function setUserInfo(person) {
        ...
        if (options.procedure === 'SIN PERMISO') {
            ...
            payload['formRegistroCitaExtranjero:nombre'] = person.firstName.toUpperCase();
            payload['formRegistroCitaExtranjero:Apellidos'] = person.lastName.toUpperCase();
    ...
    async function fetchSchedule(person) {
        ...
        payload['formRegistroCitaExtranjero:schedule_end'] = new Date(2019, 3, 30, 0, 0, 0, 0).getTime().toString();
    ...
    return { 
        process: async function() {
            ...
            if (options.procedure === 'CON PERMISO') {
                ...
                await validateNut();
                await delay(timeout);
                 select('[name=formRegistroCitaExtranjero\\:selectTramite_input]', options.procedure);
            }
 ...
 await setCaptcha();
            ...
            while (true) {
                ...
                var event = events[index];
                    console.log(event.id +"-Fecha->: "+event.start);
